...
| S1 | `vitreous/som.py`: U-matrix, seeded communities, `build_som_graph_asset`, `SomGraphProvider` satisfying the `GraphProvider` Protocol; PackWriter round-trip | Synthetic 4×4×4 SOM: K nodes, exact face-edge count, deterministic communities, `PackReader` round-trips both assets bit-exact; numpy-only import test |
