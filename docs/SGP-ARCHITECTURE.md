...
| **S5a ✅** | **`experiments/umtvit/notebooks/kaggle_umtvit_sgp.ipynb`** (+ `_build_sgp_nb.py` builder): trains/resumes UMT-ViT on HAM10000, builds SGP assets via the tested core, renders the lattice/U-matrix/BMU-replay inline, exports `som.json`+`som_bmu.bin` packs **and** a self-contained `sgp_ham10000.json` web bundle | **Done (owner runs on Kaggle):** nbformat-valid, every code cell parses, the full numpy export path composes + serializes offline; the torch surface is a thin hand-off into tested functions |
| S2 | Web decode: `SomJson` types + defensive parser, `PackClient.loadSom/loadSomBmu` (null-when-absent), uint16 decoding, mock-fixture generator emits an SGP demo pack | Fixture tests: parse-reject with named field on malformed JSON; BMU map decodes to expected values; packs without SGP assets unaffected |
| S3 | `SomBrainView` (isometric lattice, U-matrix edges, depth-scrub activation, hit-size/dead markers) + provider-discriminated mount in the workbench | Pure-function tests: isometric projection, per-depth histogram derivation, hot-set selection; ViT packs render exactly as before |
