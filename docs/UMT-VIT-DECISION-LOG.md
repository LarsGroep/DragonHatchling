...
| U7 | EuroSAT swap proof + ablation matrix + final experiment report | **Complete** (Opus agent, 2026-07-11; 139 tests incl. 3 swap-proof tests; structural EuroSAT swap proof — identical code paths for both YAMLs + synthetic EuroSAT-layout imagefolder end-to-end with zero code changes; CPU-scale ablation matrix (3 seeds × 250 steps, shapes) over baseline + 6 canonical axes; final report `docs/UMT-VIT-REPORT.md` answers the Z-axis question with measurements — documented negative result; full-scale GPU ablation flagged as owner follow-up) |
