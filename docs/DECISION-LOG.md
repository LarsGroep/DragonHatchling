...
| M6 | Gaussian Feature Field view (three.js instanced shader, vertex-shader LERP over 13 steps, ellipse hit-test, encoding legend) | **Complete** (2026-07-06; interp/hit-test unit-tested; headless-GL pixel check inconclusive under swiftshader — visual check on real GPU pending) |
| M7 | Interaction Graph (layer + unrolled modes, community hues, canvas-2D) + Latent Embedding Explorer (pooled-basis PCA cloud + CLS trail) + full four-view sync | **Complete** (2026-07-06; 42 web tests, tsc/lint/build clean) |
| M8 | Live service: §14 analyze/SSE/pack API, injected-analyzer tests, ephemeral uploads | **Complete** (2026-07-06; flow tested without torch) |
| M9 | Concepts readout in UI, docs/VITREOUS.md quick-start + deploy guide, README repositioned | **Complete** (2026-07-06). Deferred to post-v1: demo tour, run comparison, dataset-level UMAP pane (per-image PCA ships), real-GPU visual QA, actual Kaggle/Supabase/Vercel/HF provisioning (owner-run). | Pack format v1 is FROZEN (`pack_version` 1.0.0); may only *add* assets to the open asset index, never change existing layouts. |
