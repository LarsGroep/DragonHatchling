  ...
  return (
    ...
    >
      ...
      <main className="mx-auto flex max-w-[1080px] flex-col gap-3 p-3 md:p-4">
        {/*
          Synthetic-data banner. Ranked ABOVE the medical disclaimer on purpose:
          a fabricated run presented behind a careful-looking disclaimer is the
          worse failure, because the disclaimer lends it credibility the numbers
          have not earned. Shown whenever the loaded bundle is a fixture.
        */}
        {bundle?.synthetic ? (
          <div className="rounded-lg border-2 border-red-500 bg-red-50 px-3 py-2 text-[12px] text-red-900">
            <span className="font-semibold uppercase tracking-wide">Synthetic demo data — these numbers are invented.</span>{" "}
            No model produced them. The probabilities, features and benign↔malignant axis on this
            page were generated by <code className="rounded bg-red-100 px-1 font-mono">scripts/gen-lens-demo.py</code>{" "}
            to exercise the readout contract, and the thumbnails are gradients, not lesions. Nothing
            here says anything about skin cancer. Drop a real{" "}
            <code className="rounded bg-red-100 px-1 font-mono">lens_&lt;dataset&gt;.json</code> export, or
            publish one to <code className="rounded bg-red-100 px-1 font-mono">/lens/ham10000.json</code>,
            to see measured readings.
          </div>
        ) : null}
